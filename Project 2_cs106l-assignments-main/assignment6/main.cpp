...
public:
  ...
  std::optional<Course> find_course(std::string course_title)
  {
     ...
     * type. */
    std::optional<Course> target=std::nullopt;
  ...
  if (argc == 2) {
    ...
    ********************************************************/
    ...
    std::string output = course
        .transform([](const Course&c) {return "Found course: " + c.title + "," + c.number_of_units + "," + c.quarter; })
        .or_else([]()->std::optional<std::string> {return std::optional<std::string>("Course not found."); })
        .value();
    /* STUDENT_TODO */
