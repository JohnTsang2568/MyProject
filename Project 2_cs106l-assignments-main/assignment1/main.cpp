...
void parse_csv(std::string filename, std::vector<Course> &courses) {
  ...
  std::string line; Course c;
  getline(file,line);
  while(getline(file,line))
    {
      if(line.empty())continue;
    std::vector<std::string> current = split(line,',');
    c={current[0],current[1],current[2]};
    courses.push_back(c);
    }
}
...
void write_courses_offered(std::vector<Course>& all_courses) {
    ...
    for(const auto& v:all_courses)
    {
      if(v.quarter!="null")
      {
        out_file << v.title << ',' << v.number_of_units << ','  << v.quarter << "\n";
        courses_to_delete.push_back(v);
      }
    }
      out_file.close();
      for(const auto& v:courses_to_delete)
      {
        delete_elem_from_vector(all_courses,v);
      }
    }
...
void write_courses_not_offered(std::vector<Course> unlisted_courses) {
  std::ofstream out_file_2(COURSES_NOT_OFFERED_PATH);
  if(!out_file_2)return;
  out_file_2 << "Title,Number of Units,Quarter\n";
  for(const auto & v:unlisted_courses)
  {
    out_file_2 << v.title << ',' << v.number_of_units << ',' << v.quarter << "\n";
  }
  out_file_2.close();
}
